...
/**
 * expandBridgeSource - 文本展开 `// @@include <relpath>` 指令（v0.4.1 起支持任意相对路径）。
 *
 * - 默认 baseDir = bridges/，因此 `@@include ./common.js` 仍然有效（向后兼容）
 * - 也支持 `@@include ../lib/sessionTree.js` 等任意相对路径，方便把纯逻辑抽到 lib/ 共享
 * - 单遍替换，**不递归**：被嵌入的文件中的 @@include 不再展开，避免环依赖
 * - 路径解析失败时抛带 baseDir + relPath 的错误，便于诊断
 */
function expandBridgeSource(src, opts = {}){
  const baseDir = opts.baseDir || BRIDGES_DIR;
  return src.replace(/^[ \t]*\/\/\s*@@include\s+(\S+)\s*$/gm, (_match, relPath) => {
    const abs = path.resolve(baseDir, relPath);
    try {
      return fs.readFileSync(abs, 'utf8');
    } catch (err) {
      throw new Error(`expandBridgeSource: failed to include "${relPath}" (resolved to ${abs}, baseDir=${baseDir}): ${err.message}`);
    }
  });
}
...
class Session {
  ...
  log(msg){
    if (this.opts.verbose) process.stderr.write(`[deepseek-session] ${msg}\n`);
  }
  ...
  async resolveTarget(){
    ...
    const reuseAnyDeepseekTab = this.opts.reuseAnyDeepseekTab === true;
    const createUrl = this.opts.createUrl || targetUrl || null;

    let hit = pickTabMatchingProfile(tabs, this.pageProfile, { targetUrl });
    if (!hit && reuseAnyDeepseekTab) {
      hit = tabs.find((t) => /(?:^|\.)deepseek\.com/i.test((() => { try { return new URL((t && t.url) || '').hostname; } catch (_) { return ''; } })())) || null;
    }
