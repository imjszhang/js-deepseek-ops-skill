 ...
 *   爆发性大于 64KB 时，Node 会先把缓冲塞满再 pause 上游。DeepSeek 单会话历史
 *   一次性返回（带 thinking_content + 多轮长正文）很容易超过 200KB，配合
 ...
 *
 * 适用场景：批量 `node index.js list-sessions/get-session/...` 的产物归档。
 *
 * 同时注意：本函数只解决 stdout 截断问题；调用 deepseek JSON API 时 `--limit`
 * 仍受 deepseek 服务器端上限约束（建议默认 25，最大 100）。
 *
 * @example
...
function runCliToFile({ skillDir, args, outFile, entry = 'index.js', env, timeoutMs = 0 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      fd = fs.openSync(outFile, 'w', 0o600);
