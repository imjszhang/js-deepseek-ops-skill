 ...
 *     - 跳过 transformResult 的隐私 redact——destructive 调用的 prompt 原文
 *       必须保留作 audit 凭证
 *     - sideEffect === 'irreversible' 时调用前 prefetchBackup（如有）写 backup 文件
 *     - 调用后强制 writeAuditEntry，记完整 args + result 到 audit.jsonl
 */
async function runTool(browser, spec) {
  ...
  try {
    ...
    // DESTRUCTIVE + irreversible：调用前先备份当前快照
    if (destructive && sideEffect === 'irreversible' && typeof prefetchBackup === 'function') {
      try {
        ...
        process.stderr.write(`[deepseek-runTool] prefetchBackup threw: ${e && e.message}\n`);
      }
    }
  ...
  // DESTRUCTIVE：跳过 transformResult（不做隐私 redact）
  ...
  // DESTRUCTIVE：强制 audit，含完整 args + bridge result（不走 redact）
  if (destructive) {
    writeAuditEntry(runContext, {
      tool: toolName,
      method,
