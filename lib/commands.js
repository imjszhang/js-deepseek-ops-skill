...
const COMMANDS = {
  ...
  'chat-page-state': {
    ...
    toArgs: () => [{}],
    targetUrl: () => null,
    help: '读取当前 chat 页 UI 状态快照（不含 composer 草稿原文）',
  },
  'list-messages': {
    kind: 'tool',
    ...
    targetUrl: (opts, positional) => targets.chatSessionUrl({ sessionId: positional[0] }),
    help: '列出当前会话消息元数据（永不出正文）：list-messages <sessionId> [--limit N]',
  },
  'get-message': {
    kind: 'tool',
    ...
    targetUrl: (opts, positional) => targets.chatSessionUrl({ sessionId: positional[0] }),
    help: '读取单条消息：get-message <sessionId> <messageId> [--redact off|trunc|full] [--trunc-len 200]',
  },
  'streaming-status': {
    kind: 'tool',
    ...
    help: '一次性观察 assistant 是否在产出（不订阅 SSE）：streaming-status [<sessionId>]',
  },
  'chat-settings-view': {
    kind: 'tool',
    toolName: 'deepseek_chat_settings_view',
    api: 'chatSettingsView',
    pages: ['chat'],
    defaultPage: 'chat',
    argSpec: [],
  ...
  'navigate-new-chat': {
    ...
    argSpec: [],
    toNavArgs: () => ({}),
    help: '导航到 / 起新对话（仅 location.assign，不创建 sessionId）',
  },
...
function printHelp() {
  ...
  lines.push(
    ...
    '  --redact off|trunc|full  get-session / get-message 正文呈现方式（默认 off=只出 sha256 摘要）',
    '  --trunc-len <n>          --redact=trunc 时单条正文保留长度，默认 200',
    '  --content-max-len <n>    bridge 端正文硬上限，默认 60000（防 SSE 残留）',
    '  --scope main|model       chat-settings-view 的设置作用域（默认 main）',
    ...
    '  node index.js chat-page-state',
    '  node index.js list-messages d54aadf1-9f49-4f16-a8ee-a6aae6843f3a --limit 20',
    '  node index.js get-message d54aadf1-9f49-4f16-a8ee-a6aae6843f3a 12 --redact trunc',
    '  node index.js streaming-status',
    ...
    '  node index.js navigate-new-chat',
    '  # 内部踩点',
