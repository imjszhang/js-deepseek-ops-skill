 ...
 *
 * kind:
 *   - 'special':     在 cli/index.js 单独处理（doctor / dom-dump / xhr-log / export-session-local / sync-sessions / sync-session）
...
const COMMANDS = {
  ...
  'sync-sessions': {
    ...
    pages: ['home'], defaultPage: 'home',
    argSpec: [],
    help: '增量同步会话列表：sync-sessions [--limit N] [--full]',
  },
  'sync-session': {
    kind: 'special',
    ...
    help: '增量同步单会话：sync-session <sessionId> [--store-tree] [--force]',
  },
  'get-session': {
  ...
  'chat-page-state': {
    ...
    pages: ['chat'], defaultPage: 'chat',
    argSpec: [], toArgs: () => [{}], targetUrl: () => null,
    help: '读取当前 chat 页 UI 状态（含 composer chrome：mode / thinking / search / attach）',
  },
  'list-messages': {
  ...
  'dom-send-message': {
    ...
    toArgs: (opts, positional) => {
      ...
      if (opts.thinking != null) args.thinking = !!opts.thinking;
      if (opts.search != null) args.search = !!opts.search;
      return [args];
    },
    targetUrl: (opts) => opts.sessionId ? targets.chatSessionUrl({ sessionId: opts.sessionId }) : targets.homeUrl(),
    help: '[DESTRUCTIVE,COST] DOM 发消息：dom-send-message <prompt...> [--mode default|expert|vision] [--thinking|--no-thinking] [--search|--no-search] [--session <sid>] [--no-wait]',
  },
  'dom-edit-message': {
...
function parseArgv(argv) {
  const opts = {
    ...
    agent: null, comment: null, parent: null, thinking: null, search: null, mode: null,
    ...
    leaf: null,
    full: false, storeTree: false, force: false,
  };
  const positional = [];
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--no-search') opts.search = false;
    ...
    else if (a === '--store-tree') opts.storeTree = true;
...
function printHelp() {
  ...
  lines.push(
    ...
    '  --full                   sync-sessions 完整翻页并标记 disappeared',
    '  --store-tree             sync-session 把全树正文写入 tree.json',
    '  --force                  sync-session 忽略 version 短路',
    '  --pretty                 JSON 缩进 2 空格',
