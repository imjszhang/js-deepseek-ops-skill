...
const COMMANDS = {
  ...
  'delete-session': {
    kind: 'destructive', toolName: 'deepseek_delete_session', api: 'deleteSession',
    pages: ['home'], defaultPage: 'home', sideEffect: 'irreversible',
    prefetchBackup: true,
    argSpec: [{ name: 'sessionId', required: true }],
    toArgs: (opts, positional) => [{ sessionId: positional[0] }], targetUrl: () => null,
    help: '[DESTRUCTIVE,IRREVERSIBLE] 删除会话（自动备份）：delete-session <sessionId>',
  },
  'feedback-message': {
  ...
  'dom-edit-message': {
    ...
    help: '[DESTRUCTIVE,COST] DOM 编辑最后一条 USER 消息：dom-edit-message <prompt...> [--session <sid>] [--no-wait]',
  },
  'dom-regenerate-message': {
    ...
    targetUrl: (opts) => opts.session ? targets.chatSessionUrl({ sessionId: opts.session }) : null,
    help: '[DESTRUCTIVE,COST] DOM 重生最后一条 ASSISTANT 消息：dom-regenerate-message [--session <sid>] [--no-wait]',
  },
  'dom-stop-stream': {
...
function parseArgv(argv) {
  const opts = {
    ...
    noWait: false, sidTimeout: null, finishTimeout: null,
  };
  const positional = [];
...
function printHelp() {
  ...
  lines.push(
    ...
    '  node index.js delete-session <sid>           # 自动写 backup',
